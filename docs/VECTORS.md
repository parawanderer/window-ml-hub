...
it is at **3**: certificates carry a validity window (2), and there is now a `renewal` to check against (3). The
...
4. **Hello.** Rebuild the transcript from the hub name, challenge nonce, principal id, role and account id (lengths
...
5. **Channel names.** `channel_key` plus each entry's purpose and subject must give that entry's `channel`:
   HMAC-SHA256 over `"wmlhub/channel/v1" || 0x00 || purpose || 0x00 || subject`, truncated to 16 bytes.
6. **Command and result.** Open `command.sealed` as the runtime (HPKE base mode, its agreement key, info as given),
...
7. **Grant and frame.** Open `grant.sealed` as the phone (info `"wmlhub/keygrant/v1"`, signature
