...
| `http` | the connector's response-head parsing and chunked decoding never panic on arbitrary bytes, and the same body decodes the same however the reads are split |
| `box_frame` | reading a box event frame's tags never panics, and whenever the frame decodes, the kind and `info` presence read from its tags match the decoded frame |
