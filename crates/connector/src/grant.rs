...
//!
//! Most refusals are silence. The common one never reaches here at all: a command from a principal whose certificate
//! does not grant `view` fails in `Receiver::open`, so there is nothing to answer it with. The asker learns it worked
//! when the grant arrives, and that the connector is there from presence.
//!
//! A refusal for a REVOCATION reason is answered, with a sealed result naming the command's nonce and saying why
//! ([`refusal`]). A device that is not granted because the account's revocation list has gone stale should tell its
//! person that box access pauses until the revoker is back, and silence would read as a broken box instead. A device
//! that does not expect a result for `box.grant` ignores one, so this changes nothing for anything already built.

use wmlhub_keys::scope;
use wmlhub_seal::{Opened, Recipient, SealError, Sender, StreamKey, wrap_key};

use crate::relay::{Channels, KeyStart};
use crate::revoked::NotNow;
...
/// The body of the answer to a `box.grant` refused for a revocation reason: `revoked`, or `stale <ms>` with the epoch
/// milliseconds since which the account's revoker has not been heard from. Plain text, because it is the whole of a
/// connector's vocabulary for this and a device reads it to choose a sentence, not to parse a structure.
pub fn refusal(why: &NotNow) -> Vec<u8> {
    match why {
        NotNow::Revoked => b"revoked".to_vec(),
        NotNow::Stale { since_ms } => format!("stale {since_ms}").into_bytes(),
    }
}

/// Is this the command a connector answers, asked in a way that could be answered? Checked before revocations are,
/// so that a command this connector does not know stays silent however the account's list stands.
pub fn is_grant(asked: &Opened) -> bool {
    asked.body == GRANT_COMMAND.as_bytes() && asked.scope == scope::VIEW
}
...
///
/// `from` is where the current key starts on each channel. The key is generated when a connector starts and replaced
/// when a revocation names somebody new, so a grant covers one key from its first frame, and what excludes a device is
/// rotating the key rather than where a grant begins.
pub fn wrap_for(
    asked: &Opened,
    publisher: &Sender,
    key: &StreamKey,
    ...
    let edge = wrap_key(publisher, &to, &channels.edge, key, from.edge, now_ms)?;
    let sample = wrap_key(publisher, &to, &channels.sample, key, from.sample, now_ms)?;
    Ok(Ok([edge, sample]))
}
...
mod tests {
    ...
    const START: KeyStart = KeyStart { edge: 1, sample: 1 };
    ...
    fn a_device_that_may_view_is_wrapped_the_key_for_both_channels() {
        ...
        let wrapped = wrap_for(
            &asked(scope::VIEW, GRANT_COMMAND.as_bytes(), vec![3; 32]),
            &publisher,
            &key,
            &channels(),
            START,
    ...
    fn anything_else_is_refused_rather_than_guessed_at() {
        ...
        let publisher = Sender { identity: &identity, chain: &[] };
        let key = StreamKey::from_bytes([2; 32]);
        let refuse = |asked: Opened| wrap_for(&asked, &publisher, &key, &channels(), START, NOW).unwrap().unwrap_err();
